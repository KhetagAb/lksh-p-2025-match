...
type (
	RegisterPlayerService interface {
		RegisterUser(ctx context.Context, name string, tgUsername string, tgId int) (*int, error)
	}
...
func (h *RegisterPlayerHandler) RegisterUser(ectx echo.Context, params server.RegisterPlayerParams) error {
	...
	playerId, err := h.registerPlayerService.RegisterUser(ctx, params.Name, params.TgUsername, params.TgId)
