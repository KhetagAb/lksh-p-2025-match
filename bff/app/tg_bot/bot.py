...
def sign_up_to_sport(sport):
    pass


def registration_person(username):
    pass


def get_list_of_sports(sport):
    pass


def make_sports_buttons(pref):
...
async def answer_to_buttons(mess):
    for sport in get_all_sports():
        ...
        if 'list_' + sport == mess.text:
            await bot.send_message(mess.chat.id, get_)
    await bot.send_message(mess.chat.id, "I don't understand you.")


@bot.message_handler(commands=['list'])
async def list_(mess):
    await bot.send_message(mess.chat.id, "You can sign up for these sports", reply_markup=make_sports_buttons('list_'))
