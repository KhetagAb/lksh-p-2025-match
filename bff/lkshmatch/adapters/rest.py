...
from bff.lkshmatch.adapters.core import (GetPlayersBySportSections,
                                         GetSportSections,
                                         NameTeamReserveError, PlayerAddInfo,
                                         PlayerAlreadyInTeam, PlayerNotFound,
                                         PlayerRegisterInfo,
                                         RegisterPlayerInSportSection,
                                         RegisterUser, SportSection, Team,
                                         UnknownError, ValidateRegisterUser)
    ...
    async def validate_register_user(self, user: PlayerAddInfo) -> str:
        async with aiohttp.ClientSession() as session:
            ...
            response = await session.get(
                f"{API_URL}/validate_register_user", params=query
            )
...
class RestRegisterUser(RegisterUser):
    async def register_user(self, user: PlayerAddInfo) -> PlayerRegisterInfo:
        async with aiohttp.ClientSession() as session:
            query = {"tg_username": user.tg_username, "tg_id": user.tg_id}
            response = await session.get(
                f"{API_URL}/register_user", params=query
            )
...
class RestGetPlayersBySportSections(GetPlayersBySportSections):
    async def get_players_by_sport_sections(
        self, section: SportSection
    ) -> list[PlayerRegisterInfo]:
        async with aiohttp.ClientSession() as session:
            query = {"name_section": section.en_name}
            response = await session.get(
                f"{API_URL}/get_players_by_sport_sections", params=query
            )
            if response.status != 200:
                raise UnknownError
            ...
            data = await response.json()
            return [
                PlayerRegisterInfo(name, id_player) for name, id_player in data
            ]


class RestRegisterPlayerInSportSection(RegisterPlayerInSportSection):
    async def register_player_in_sport_sectoin(
        self, section: SportSection, user: PlayerRegisterInfo
    ) -> None:
        async with aiohttp.ClientSession() as session:
            query = {"name_section": section.en_name, "user_id": user.id}
            response = await session.get(
                f"{API_URL}/register_player_in_sport_sectoin", params=query
            )
            if response.status != 200:
                raise UnknownError
